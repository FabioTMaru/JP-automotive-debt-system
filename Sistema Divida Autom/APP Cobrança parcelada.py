...
import platform
import shutil
import math  # Importante para calcular arredondamento de parcelas
...
class RelatoriosPDF:
    ...
    # --- NOVO RELATÓRIO DE PREVISÃO ---
    def gerar_relatorio_previsao_mensal(self, dados_agrupados):
        filename = "Relatorio_Previsao_Recebimento.pdf"
        doc = SimpleDocTemplate(filename, pagesize=A4)
        elementos = []

        # Título
        style_azul = ParagraphStyle('Azul', parent=self.styles['Heading1'], alignment=1, fontSize=16, spaceAfter=20,
                                    textColor=colors.darkblue)
        elementos.append(Paragraph("PREVISÃO DE RECEBIMENTOS - DÍVIDAS MÊS A MÊS", style_azul))
        elementos.append(Paragraph(f"Gerado em: {datetime.now().strftime('%d/%m/%Y')}", self.styles['Normal']))
        elementos.append(Spacer(1, 20))

        # Tabela
        data = [["MÊS DE REFERÊNCIA", "VALOR PREVISTO (¥)"]]
        total_geral_previsto = 0

        for mes_ano, valor in dados_agrupados:
            data.append([mes_ano, Utils.formatar_moeda(valor)])
            total_geral_previsto += valor
        ...
        # Total
        elementos.append(Paragraph(f"<b>TOTAL PREVISTO FUTURO: {Utils.formatar_moeda(total_geral_previsto)}</b>",
                                   self.styles['Normal']))
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.setup_ui()
        self.carregar_dados()
        self.verificar_lembrete_backup()
    ...
    def setup_ui(self):
        ...
        self.frame_menu.grid_rowconfigure(0, weight=0)
        self.frame_menu.grid_rowconfigure(1, weight=1)
        self.frame_menu.grid_rowconfigure(2, weight=0)
    ...
    def criar_menu_lateral(self):
        ctk.CTkLabel(self.frame_menu, text="JPY SYSTEM ¥", font=("Arial", 26, "bold")).grid(row=0, column=0,
                                                                                            pady=(30, 10))
        self.scroll_menu = ctk.CTkScrollableFrame(self.frame_menu, fg_color="transparent")
        self.scroll_menu.grid(row=1, column=0, sticky="nsew", padx=0, pady=0)
        ...
        for text, cmd, color in btn_configs:
            if text == "separator":
                ctk.CTkFrame(self.scroll_menu, height=2, fg_color="gray50").pack(fill="x", padx=15, pady=10)
            else:
                ctk.CTkButton(self.scroll_menu, text=text, command=cmd, fg_color=color, font=("Arial", 14, "bold"),
                              height=40).pack(fill="x", padx=15, pady=5)

        ctk.CTkLabel(self.scroll_menu, text="Relatórios PDF", text_color="gray").pack(pady=(20, 5))
        ctk.CTkButton(self.scroll_menu, text="📄 Do Mês Atual", command=self.imprimir_relatorio_mensal, fg_color="#555",
                      height=30).pack(fill="x", padx=30, pady=2)
        ctk.CTkButton(self.scroll_menu, text="📄 Devedores do Mês", command=self.imprimir_relatorio_devedores,
                      fg_color="#c0392b", height=30).pack(fill="x", padx=30, pady=2)

        # --- NOVO BOTÃO ---
        ctk.CTkButton(self.scroll_menu, text="📄 Previsão Recebimento", command=self.imprimir_relatorio_previsao,
                      fg_color="#2980b9", height=30).pack(fill="x", padx=30, pady=2)

        ctk.CTkButton(self.scroll_menu, text="📄 Dívida Total", command=self.imprimir_relatorio_divida, fg_color="#555",
                      height=30).pack(fill="x", padx=30, pady=2)

        ctk.CTkFrame(self.scroll_menu, height=2, fg_color="gray50").pack(fill="x", padx=15, pady=15)
        ctk.CTkButton(self.scroll_menu, text="💾 Backup de Segurança", command=self.realizar_backup, fg_color="#2c3e50",
                      hover_color="black").pack(fill="x", padx=15, pady=(0, 20))

        self.frame_data = ctk.CTkFrame(self.frame_menu, fg_color="transparent")
        self.frame_data.grid(row=2, column=0, pady=20, sticky="ew")
    ...
    def modal_listar_pagamentos(self, venda_id):
        ...
        def editar():
            # CORREÇÃO: Pegar o ID ANTES de destruir a janela
            sel = tree.selection()
            if not sel:
                return messagebox.showwarning("Aviso", "Selecione um pagamento.")

            id_pagamento = tree.item(sel[0])['values'][0]
            jan.destroy()
            self.modal_editar_pagamento_unico(id_pagamento)
    ...
    # --- NOVO MÉTODO: RELATÓRIO PREVISÃO ---
    def imprimir_relatorio_previsao(self):
        sql = "SELECT id, valor_mensal, valor_venda, entrada FROM vendas"
        vendas = self.db.query(sql)

        # Dicionário para agrupar por mês/ano. Ex: {'03/2026': 50000, '04/2026': 50000}
        previsao_dict = {}

        data_atual = datetime.now()

        for v in vendas:
            vid, mensal, total, entrada = v
            ...
            res_pagos = self.db.query("SELECT SUM(valor_pago) FROM pagamentos WHERE venda_id=?", (vid,))
            pago_extra = res_pagos[0][0] if res_pagos[0][0] else 0
            saldo = total - (entrada + pago_extra)

            if saldo <= 0: continue

            # Calcula quantas parcelas faltam
            parcelas_restantes = math.ceil(saldo / mensal)

            # Projeta para o futuro
            for i in range(1, parcelas_restantes + 1):
                # Data da parcela futura (meses a partir de agora)
                data_parcela = data_atual + relativedelta(months=i)
                chave_mes = data_parcela.strftime("%m/%Y")
                chave_sort = data_parcela.strftime("%Y%m")  # Chave para ordenação correta (YYYYMM)

                # Define valor da parcela (a última pode ser menor que o valor mensal cheio)
                valor_desta_parcela = min(mensal, saldo)
                saldo -= valor_desta_parcela

                # Armazena usando chave composta para ordenar depois: (YYYYMM, MM/YYYY)
                chave_composta = (chave_sort, chave_mes)

                if chave_composta in previsao_dict:
                    previsao_dict[chave_composta] += valor_desta_parcela
                else:
                    previsao_dict[chave_composta] = valor_desta_parcela

        if not previsao_dict:
            return messagebox.showinfo("Info", "Não há previsões futuras de recebimento.")

        # Ordena cronologicamente e prepara para o PDF
        # O sorted() vai usar o primeiro item da tupla (YYYYMM) para ordenar
        dados_ordenados = []
        for chave, valor in sorted(previsao_dict.items()):
            mes_formatado = chave[1]  # Pega o "MM/YYYY"
            dados_ordenados.append([mes_formatado, valor])

        self.pdf_manager.gerar_relatorio_previsao_mensal(dados_ordenados)
